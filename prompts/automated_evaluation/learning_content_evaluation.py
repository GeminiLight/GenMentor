...
{{
    "content_quality": {{
        "agreement": "Detailed agreement points on content quality (max 50 words).",
        "disagreement": "Detailed disagreement points on content quality (max 50 words).",
        "score": content_quality_score
    }},
    "goal_relevance": {{
        "agreement": "Detailed agreement points on goal relevance (max 50 words).",
        "disagreement": "Detailed disagreement points on goal relevance (max 50 words).",
        "score": goal_relevance_score
    }},
    "engagement": {{
        "agreement": "Detailed agreement points on engagement (max 50 words).",
        "disagreement": "Detailed disagreement points on engagement (max 50 words).",
        "score": engagement_score
    }},
    "personalization": {{
        "agreement": "Detailed agreement points on personalization (max 50 words).",
        "disagreement": "Detailed disagreement points on personalization (max 50 words).",
...
learning_content_evaluator_system_prompt = """
You are a Learning Content Evaluator in a Goal-oriented Learning in Intelligent Tutoring System. 
Your task is to evaluate generated learning content based on four key criteria: Content Quality, Goal Relevance, Engagement, and Personalization. Provide a score from 1 to 5 for each criterion, with detailed justifications for both agreement and disagreement points. The evaluation ensures that the content is accurate, goal-aligned, engaging, and tailored to the learner's profile.
...
1. **Content Quality**:
   - **Definition**: The accuracy, orignization, clarity, coherence, and depth of the content.
   - **Scoring**:
     - **5**: Highly accurate, well-structured, and easy to understand.
     - **4**: Mostly accurate with minor improvements needed in clarity or coherence.
     - **3**: Moderately clear and structured, but with noticeable inaccuracies or inconsistencies.
     - **2**: Poorly structured or unclear with significant factual errors.
     - **1**: Inaccurate, unclear, and disorganized.
...
- Justify your scores with detailed points of agreement and disagreement.
- Include additional feedback and suggestions for improvement.
"""

learning_content_evaluator_task_prompt = """
Evaluate the following learning content based on the criteria of Content Quality, Goal Relevance, Engagement, and Personalization. Provide a score from 1 to 5 for each criterion, along with a detailed explanation for agreement and disagreement points.
...
LEARNING_CONTENT_EVALUATION_OUTPUT_FORMAT
"""
learning_content_evaluator_task_prompt = learning_content_evaluator_task_prompt.replace(
    "LEARNING_CONTENT_EVALUATION_OUTPUT_FORMAT", learning_content_evaluation_output_format_with_title
)

learning_content_evaluator_task_prompt_batch = """
...
LEARNING_CONTENT_EVALUATION_OUTPUT_FORMAT_BATCH
"""
learning_content_evaluator_task_prompt_batch = learning_content_evaluator_task_prompt_batch.replace(
